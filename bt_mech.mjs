...
import { loopJson, log, warn, DAG, DDAG, DDDAG, BR, newRow, td, tdr, tdv, tdh } from './bt_utils.mjs';
...
function listMechs( cls, list ) {
   ...
   log( `: ${DDDAG} Mech has the BLACKLISTED tag` );
}
...
function getMechName( e ) {
	const name = e.Name;
	if (!e.MechTags.items.includes( "BLACKLISTED" )) return name;
	return name + DDDAG;
}
